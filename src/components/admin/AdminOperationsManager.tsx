...
export function AdminOperationsManager({ ownStudioId, activePeriod: serverActivePeriod }: Props) {
  const [transactions, setTransactions] = useState<Transaction[]>([])
  const [clients, setClients] = useState<Client[]>([])
  const [activePeriod, setActivePeriod] = useState(serverActivePeriod)
  ...
  const fetchData = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/admin/transactions?studioId=${activeStudioId}`)
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (transactionsToInsert.length > 0) {
        ...
        if (result.error) {
          ...
          if (errors.length > 0) {
            toast({
              ...
              description: `${transactionsToInsert.length} ok, ${errors.length} errores. Ver consola.`
            })
            console.error('Errores de importación:', errors)
          } else {
            toast({ title: 'Importado', description: `${transactionsToInsert.length} transacciones importadas` })
          }
        }
      } catch {
      ...
      toast({ variant: 'destructive', title: 'Error', description: `No se pudo importar ninguna fila. ${errors.length} errores.` })
      console.error('Errores de importación:', errors)
    }
  ...
  if (!activeStudioId) {
    return (
      <div className="flex items-center gap-3 p-6 bg-amber-50 rounded-lg border border-amber-200">
        <AlertCircle className="h-5 w-5 text-amber-600" />
        <div>
          <p className="font-medium text-amber-800">Sin estudio seleccionado</p>
